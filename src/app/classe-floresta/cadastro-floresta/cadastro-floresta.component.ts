...
import { ClasseFlorestaModule } from './../classe-floresta.module';
...
import { MenuEmpresa } from 'src/app/core/model';
...
export class CadastroFlorestaComponent implements OnInit {
  ...
  constructor(
    ...
    private empresaService: CadempresaService,
    private title: Title,
  ...
  atualizarFloresta(form: FormControl){
    this.florestaService.atualizar(this.classeflorestaSalva)
     .then(classefloresta => {
        ...
        this.router.navigate(['/cadastro-floresta']);
     })
     .catch(erro => this.errorHandler.handle(erro));
  }
